...
def BuildPackage(DirPath, IsMainPkg=True):
  if IsMainPkg:
...
def download_AUR_Pkg(PkgName, IsMainPackage=True):
 ...
 try:
  ...
  if IsMainPackage:
       depenChoice = input("Would you like to download AUR Dependencies? Y/n ").strip()
  ...
  if depenChoice == "Y" and IsMainPackage:
    FoundDepends = DownloadDependencies(PkgName)
    FoundDepends.reverse() 
    for Depend in FoundDepends:
      download_AUR_Pkg(Depend, IsMainPackage=False)
  ...
  if not IsMainPackage:
     ...
     BuildPackage(ExtractedFolderPath, IsMainPkg=False)
...
if __name__ =="__main__":
 ...
 while True:
  try:
   if TargetPackage:
     download_AUR_Pkg(TargetPackage, IsMainPackage=True)
